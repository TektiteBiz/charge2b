...
/* Type of page erasing:
       ...
       EE_CONDITONAL_ERASE --> pages to erase are erased only if not fully
   erased */
typedef enum { EE_FORCED_ERASE, EE_CONDITIONAL_ERASE } EE_Erase_type;

/* Masks of EE_Status return codes */
#define EE_STATUSMASK_ERROR                                                   \
  (uint16_t)0x00FFU /*!< Mask on EE_Status return code, selecting error codes \
                     */
#define EE_STATUSMASK_CLEANUP                                             \
  (uint16_t)0x0100U /*!< Mask on EE_Status return code, selecting cleanup \
                       request codes */
