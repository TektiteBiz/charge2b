...
EE_Status EE_Init(EE_Erase_type EraseType) {
  ...
  for (page = START_PAGE; page < (START_PAGE + PAGES_NUMBER); page++) {
    ...
    for (varidx = 0U; varidx < PAGE_SIZE; varidx += EE_ELEMENT_SIZE) {
      /*
        ...
        we save the address read to set its content to 0 in case it triggered an
        NMI (see NMI_Handler in stm32lxxx_it.c). In the rest of the program, we
        do nothing in case a NMI is triggers by a reading because no NMI should
        be triggered since we have cleanup the EEPROM emulated. By the way,
        there is still the CRC code associated to each EEPROM line that allows
        to verify its valid state.
      */
  ...
  if (nbactivereceivepage == 0U) {
    ...
    if (nbvalidpage > 0U) {
      ...
      if (GetPageState(PAGE_ADDRESS(PREVIOUS_PAGE(firstvalidpage))) ==
          STATE_PAGE_ERASING) {
        if (SetPageState(FOLLOWING_PAGE(lastvalidpage), STATE_PAGE_RECEIVE) !=
            EE_OK) {
  ...
  for (page = START_PAGE; page < (START_PAGE + PAGES_NUMBER); page++) {
    ...
    /* Keep index of last valid page, will be required in case no active page is
       found */
    else if (pagestatus == STATE_PAGE_VALID) {
      lastvalidpage = page;
    }
  ...
  for (varidx = PAGE_HEADER_SIZE; varidx < PAGE_SIZE;
    ...
    addressvalue =
        (*(__IO EE_ELEMENT_TYPE*)(PAGE_ADDRESS(ubCurrentActivePage) + varidx));
  ...
  /*********************************************************************/
  /* Check consistency of pages sequence: one active page, optionally some valid
   * pages before */
 ...
 * @param VirtAddress Variable virtual address on 16 bits (can't be 0x0001 or
 * 0xFFFE)
...
EE_Status EE_CleanUp(void) {
  ...
  for (firstpage = START_PAGE; firstpage < (START_PAGE + PAGES_NUMBER);
    ...
    if (firstpagestatus == STATE_PAGE_ERASING) {
      for (page = (firstpage + 1U); page < (firstpage + (PAGES_NUMBER / 2U));
           page++) {
        pageaddress = PAGE_ADDRESS(page);
        pagestatus = GetPageState(pageaddress);
